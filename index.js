// index.js — WhatsApp ↔ Kayako bridge (persistent thread map + deterministic Message-ID)
//
// - Twilio inbound -> SendGrid email to Kayako (stores a Message-ID per phone;
//   sets In-Reply-To/References on the next message; optional subject case token)
// - SendGrid inbound -> Twilio WhatsApp (strips signatures/quotes, relays attachments via temp URLs)
//
// Requires env:
//   SENDGRID_API_KEY
//   MAIL_TO=hello@stickershop.co.uk
...
//   KAYAKO_USERNAME=...   (optional; enables case lookup for subject token)
//   KAYAKO_PASSWORD=...   (optional)
//   KAYAKO_FROM_ALLOWLIST=hello@stickershop.co.uk (optional; CSV)
//   SG_INBOUND_SECRET=... (optional; adds simple auth to /sg-inbound)
//   SERVICE_BASE_URL=https://your-render-url (optional; for /file links)
//   MAP_PATH=/data/thread_map.json  (recommended; persists on Render disk)
...
const SERVICE_BASE   = process.env.SERVICE_BASE_URL || '';
...
// ---------- Boot-time checks ----------
function ensureThreadMapExists() {
  try {
    fs.mkdirSync(path.dirname(MAP_PATH), { recursive: true });
    if (!fs.existsSync(MAP_PATH)) {
      fs.writeFileSync(MAP_PATH, '{}', 'utf8');
      console.log('🗂 Created empty thread map at', MAP_PATH);
    } else {
      console.log('🗂 Using existing thread map at', MAP_PATH);
    }
    // simple write test
    fs.writeFileSync(path.join(path.dirname(MAP_PATH), '.rw_test'), 'ok');
  } catch (e) {
    console.warn('⚠️ Could not init thread map:', e.message);
  }
}
ensureThreadMapExists();
...
// (Optional) subject token: latest open case for this identity
...
app.post('/incoming-whatsapp', async (req, res) => {
  ...
  const mapBefore = loadThreadMap();
  const prev = mapBefore[phoneKey];

  // Create our own deterministic Message-ID and SAVE IT *before* sending
  const myMsgId = ((crypto.randomUUID && crypto.randomUUID()) || crypto.randomBytes(16).toString('hex')) + '@' + FROM_DOMAIN;
  ...
  const msg = {
    ...
    headers: {
      ...
      'Message-ID': `<${myMsgId}>`,
      ...(prev?.lastMsgId ? {
        'In-Reply-To': `<${prev.lastMsgId}>`,
        'References' : `<${prev.lastMsgId}>`
      } : {})
    }
  };

  // Persist anchor right now so the *next* message can reference it
  const mapNow = loadThreadMap();
  mapNow[phoneKey] = { lastMsgId: myMsgId, updatedAt: Date.now() };
  saveThreadMap(mapNow);
  console.log(`🧵 Saved anchor for ${phoneKey}: <${myMsgId}> → ${MAP_PATH}`);
...
// ---------- Debug routes ----------
app.get('/debug/thread-map', (req, res) => {
  try {
...
app.post('/debug/thread-map/reset', (req, res) => {
  try {
    fs.mkdirSync(path.dirname(MAP_PATH), { recursive: true });
    fs.writeFileSync(MAP_PATH, '{}', 'utf8');
    res.send('reset');
